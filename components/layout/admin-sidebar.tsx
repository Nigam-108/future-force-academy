...
export function AdminSidebar({ isOpen, onClose }: Props) {
  ...
  return (
    <aside
      className={`flex flex-col w-64 shrink-0 fixed top-0 left-0 h-screen border-r border-slate-800 bg-slate-950 z-30 transition-transform duration-300 ease-in-out ${
    ...
    >
              ...
              <div key={group.groupLabel} className="mb-2">
                <button
                  ...
                  className={`flex w-full items-center justify-between rounded-xl px-3 py-2 transition-colors ${
                    groupHasActive && !isOpen_ ? "bg-white/5" : "hover:bg-white/5"
                  }`}
                >
      ...
      <div className="shrink-0 border-t border-slate-800 p-4">
        <div className="flex items-center gap-3 rounded-xl bg-white/5 px-3 py-2.5">
          <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-blue-600 text-xs font-bold text-white">A</div>
          <div className="min-w-0">
            <p className="truncate text-xs font-semibold text-white">Main Admin</p>
            <p className="truncate text-[10px] text-slate-400">admin@futureforceacademy.com</p>
          </div>
        </div>
      </div>
    </aside>
